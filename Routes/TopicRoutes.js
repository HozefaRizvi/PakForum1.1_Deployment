...
router.post("/add-topic", auth, async (req, res) => {
    const { topicname, TopicContent, TopicTags, TopicPhotoUri } = req.body;
    try {
...
router.post("/like-topic/:topicId", auth, async (req, res) => {
    ...
    try {
        const user = req.user;
        const existingLike = await TopicLike.findOne({ topicId, userId: user.userId });

        if (existingLike) return res.status(400).json({ message: "You have already liked this topic" });
...
router.post("/report-topic/:topicId", auth, async (req, res) => {
    ...
    try {
        const user = req.user;
        const existingReport = await TopicReport.findOne({ topicId, userId: user.userId });
        if (existingReport) return res.status(400).json({ message: "You have already reported this topic" });
...
router.delete("/delete-topic/:topicId", auth, async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Topic and associated data deleted successfully" });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Server error" });
    }
